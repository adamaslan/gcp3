...
async def main() -> None:
    ...
    if result["drifting"]:
        ...
        print('  # Required: export GCP_PROJECT_ID and BACKEND_URL first')
        print('  SECRET=$(gcloud secrets versions access latest --secret=SCHEDULER_SECRET --project=$GCP_PROJECT_ID)')
        print(f'  curl -X POST "$BACKEND_URL/admin/seed-etf-history?force=true&symbols={offenders}" \\')
        print('       -H "X-Scheduler-Token: $SECRET"')
