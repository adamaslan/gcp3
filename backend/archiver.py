...
def archive_expired_docs(docs: list[tuple[str, dict]], source: str = "gcp3_cache") -> int:
    ...
    Returns:
        Number of rows written. Returns 0 (never raises) on any failure so
        that a data-lake outage never blocks the purge from running.
    """
    if not docs:
        return 0
    ...
    try:
        ...
        logger.warning("archiver: archive_expired_docs failed, purge continues: %s", exc)
        return 0
